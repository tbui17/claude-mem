...
${pc.bold('Install Commands')} (no Bun required):
  ...
  ${pc.cyan('npx claude-mem install --provider claude|gemini|openrouter|openai-compatible|opencode-go')}   Set LLM provider non-interactively
...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'gemini' && provider !== 'openrouter' && provider !== 'openai-compatible' && provider !== 'opencode-go') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, gemini, openrouter, openai-compatible, opencode-go`);
    process.exit(1);
  }
  return {
