...
export function loadClaudeMemEnv(): ClaudeMemEnv {
  ...
  try {
    ...
    if (parsed.OPENCODE_GO_API_KEY) result.OPENCODE_GO_API_KEY = parsed.OPENCODE_GO_API_KEY;
...
export function saveClaudeMemEnv(env: ClaudeMemEnv): void {
  ...
  if (env.OPENCODE_GO_API_KEY !== undefined) {
    if (env.OPENCODE_GO_API_KEY) {
      updated.OPENCODE_GO_API_KEY = env.OPENCODE_GO_API_KEY;
    } else {
      delete updated.OPENCODE_GO_API_KEY;
    }
  }
...
export function buildIsolatedEnv(includeCredentials: boolean = true): Record<string, string> {
  ...
  if (includeCredentials) {
    ...
    if (credentials.OPENCODE_GO_API_KEY) {
      isolatedEnv.OPENCODE_GO_API_KEY = credentials.OPENCODE_GO_API_KEY;
    }
